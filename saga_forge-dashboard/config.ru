...
E.create!(saga_class: "DemoSaga", correlation_id: "demo-101",
...
E.create!(saga_class: "DemoSaga", correlation_id: "demo-201",
...
E.create!(saga_class: "DemoSaga", correlation_id: "demo-301",
...
E.create!(saga_class: "DemoSaga", correlation_id: "demo-401",
...
E.create!(saga_class: "DemoSaga", correlation_id: "demo-501",
...
# (review_passed or more_info_needed -> jump completed) -> completed
...
E.create!(saga_class: "OrderSaga", correlation_id: "order-1001",
...
# Running: further along — settled, then a more_info_needed review event landed,
# so the timeline shows the full order_placed -> settled -> review path.
...
E.create!(saga_class: "OrderSaga", correlation_id: "order-1002",
...
E.create!(saga_class: "OrderSaga", correlation_id: "order-2001",
...
E.create!(saga_class: "OrderSaga", correlation_id: "order-2002",
...
E.create!(saga_class: "OrderSaga", correlation_id: "order-3001",
...
E.create!(saga_class: "OrderSaga", correlation_id: "order-4001",
...
E.create!(saga_class: "OrderSaga", correlation_id: "order-5001",
...
E.create!(saga_class: "OrderSaga", correlation_id: "order-6001",
...
E.create!(saga_class: "OrderSaga", correlation_id: "order-7001",
...
# Backfill the persisted lifecycle timestamps the dashboard reads (added in the
# structural-dedup refactor). The finalized/active split keys off finalized_at:
# terminal sagas (a finish state, or an operator cancel) carry it; everything
# still in flight — running, stalled, suspended, compensating — carries only
# last_active_at, which drives the age meters. update_columns preserves the
# seeded created_at/updated_at above.
FINALIZED_STATES = %w[demo_complete completed cancelled].freeze
S.find_each do |s|
  if FINALIZED_STATES.include?(s.current_state)
    s.update_columns(finalized_at: s.updated_at, last_active_at: s.updated_at)
  else
    s.update_columns(last_active_at: s.updated_at)
  end
end
